...
MODES=(single_sync single_async multi_sync multi_async)

for mode in "${MODES[@]}"; do
  PROFILE=noattack   LOSS_NAME="${LOSS_NAME}"   MODE_SET="${mode}"   SUITE_SEED="${SUITE_SEED}"   RUNTIME_DEVICE="${RUNTIME_DEVICE}"   TRAIN_OPTIMIZER="${TRAIN_OPTIMIZER}"   BASE_OUTPUT_ROOT="${OUTPUT_PREFIX}/${mode}/${SUITE_SEED}"   PROJECT_NAME="re_fl_noattack_${mode}_adam"   bash scripts/run_controlled_suite.sh
