...
```bash
conda run -n torch_env python -m fedlab.entrypoints.train --config configs/rawdata2_soteriafl.yaml
```

Run a short smoke test with overrides:

```bash
conda run -n torch_env python -m fedlab.entrypoints.train --config configs/rawdata2_soteriafl.yaml --override federated.rounds=2 --override attack.frequency_rounds=1 --override attack.steps=1
```
...
```bash
conda run -n torch_env python -m fedlab.entrypoints.train --config configs/rawdata2_fedlab_topk.yaml
```
