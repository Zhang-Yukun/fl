...
def test_build_client_split_payloads_is_deterministic_and_complete():
    ...
    first = build_client_split_payloads(train_images, train_labels, test_images, test_labels, num_clients=3, val_ratio=0.5, seed=2026)
    second = build_client_split_payloads(train_images, train_labels, test_images, test_labels, num_clients=3, val_ratio=0.5, seed=2026)

    assert set(first) == {"client1", "client2", "client3"}
...
def test_save_client_split_payloads_writes_expected_layout(tmp_path):
    ...
    assert (tmp_path / "clients" / "client1" / "train.pt").exists()
    assert (tmp_path / "clients" / "client2" / "val.pt").exists()
    assert (tmp_path / "clients" / "client3" / "test.pt").exists()
    loaded = torch.load(tmp_path / "clients" / "client2" / "train.pt", map_location="cpu", weights_only=False)
