...
import fedlab.utils.runtime as runtime_module
...
def test_seed_cuda_device_targets_only_requested_device(monkeypatch):
    ...
    monkeypatch.setattr(runtime_module.torch.cuda, "device", lambda device: _CudaDeviceContext(device))
    monkeypatch.setattr(runtime_module.torch.cuda, "manual_seed", lambda seed: calls.append(("manual_seed", int(seed))))
    monkeypatch.setattr(runtime_module.torch.cuda, "manual_seed_all", lambda seed: calls.append(("manual_seed_all", int(seed))), raising=False)
...
def test_configure_random_seed_uses_runtime_device(monkeypatch):
    ...
    monkeypatch.setattr(runtime_module, "setup_seed", fake_setup_seed)
    monkeypatch.setattr(runtime_module.torch.cuda, "is_available", lambda: True)

    runtime_module.configure_random_seed({
